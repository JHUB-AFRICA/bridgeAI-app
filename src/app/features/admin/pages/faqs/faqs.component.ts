        ...
        <table class="data-table">
          ...
          <tbody>
            <tr *ngFor="let faq of faqs()">
              <td class="title-cell">{{ faq.question }}</td>
              <td>{{ faq.audience }}</td>
              <td>
                <span class="status-badge" [class.published]="faq.is_published">
              ...
              <td class="actions-cell">
                ...
                <button class="btn-icon delete" (click)="deleteFaq(faq.id)" title="Delete">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16">
            ...
            <tr *ngIf="faqs().length === 0">
              <td colspan="4" class="empty-state">No FAQs found.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="modal-overlay" *ngIf="showModal" (click)="closeModal()">
        <div class="modal" (click)="$event.stopPropagation()">
          ...
          <div class="modal-body">
            <form (ngSubmit)="saveFaq()" #faqForm="ngForm">
              <div class="form-group">
                <label>Question *</label>
                <input type="text" [(ngModel)]="formData.question" name="question" required class="form-control" />
              </div>
              <div class="form-group">
                <label>Answer *</label>
                <textarea [(ngModel)]="formData.answer" name="answer" required class="form-control" rows="4"></textarea>
              </div>
              <div class="form-row">
                <div class="form-group">
                  <label>Audience *</label>
                  <select [(ngModel)]="formData.audience" name="audience" required class="form-control">
              ...
              <div class="form-actions">
                ...
                <button type="submit" class="btn-primary" [disabled]="faqForm.invalid">
...
export class AdminFaqsComponent implements OnInit {
  ...
  protected editingFaq: FAQ | null = null;
  ...
  private loadFaqs(): void {
    this.faqService.getFaqs().subscribe({
      ...
      error: () => {
        this.faqs.set([]);
        this.notificationService.showError('Failed to load FAQs');
      }
    });
  }
  ...
  saveFaq(): void {
    ...
    if (this.editingFaq) {
      this.faqService.updateFaq(this.editingFaq.id!, data).subscribe({
        ...
        error: () => {
          this.notificationService.showError('Failed to update FAQ');
        }
      });
    } else {
      this.faqService.createFaq(data).subscribe({
        ...
        error: () => {
          this.notificationService.showError('Failed to create FAQ');
        }
      });
    }
  }
  ...
  deleteFaq(id: number | undefined): void {
    ...
    if (confirm('Are you sure you want to delete this FAQ?')) {
      this.faqService.deleteFaq(id).subscribe({
        ...
        error: () => {
          this.notificationService.showError('Failed to delete FAQ');
        }
      });
    }
  }
